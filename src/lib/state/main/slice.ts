...
export const searchMainstemsCQL = createAsyncThunk(
    ...
    async (query: string) => {
        const response = await fetch(
            `https://reference-geoconnex-us-414886575015.us-central1.run.app/collections/mainstems/items?filter=name_at_outlet+ILIKE+'%${query}%'+OR+uri+ILIKE+'%mainstems/${query}%'&f=json`
        );
        const data = await response.json();
        return data;
    }
